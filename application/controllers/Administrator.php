...
class Administrator extends CI_Controller {
    public function tambahAdmin()
    {
        $data = [
            'username'  => 'admin2',
            'password'  => hash("sha512", md5('admin2')),
            'nama_lengkap'  => 'admin',
    ...
    // Modul Controller Beasiswa
    function beasiswa () {
        cek_session_akses('beasiswa',$this->session->id_session);
        $data['record'] = $this->db->get('beasiswa')->result();
		$this->template->load('administrator/template','administrator/mod_beasiswa/beasiswa', $data);
    }

    function penerimaBeasiswa () {
        cek_session_akses('beasiswa',$this->session->id_session);
        $data['record'] = $this->db->select(['nim', 'identitas_mahasiswa.nama AS nama', 'asal_kampus', 'anggaran', 'tahap', 'status', 'identitas_wali.id AS id_wali', 'identitas_mahasiswa.id AS id_mahasiswa', 'dokumen_beasiswa.id AS id_dokumen', 'beasiswa.id AS id_beasiswa'])
                                    ->from('penerima_beasiswa')
                                    ->join('identitas_wali', 'identitas_wali.id = penerima_beasiswa.id_biodata_wali')
                                    ->join('identitas_mahasiswa', 'identitas_mahasiswa.id = penerima_beasiswa.id_biodata_mahasiswa')
                                    ->join('dokumen_beasiswa', 'dokumen_beasiswa.id = penerima_beasiswa.id_dokumen')
                                    ->join('beasiswa', 'beasiswa.id = penerima_beasiswa.id_beasiswa')
                                    ->get()
                                    ->result();
		$this->template->load('administrator/template','administrator/mod_beasiswa/penerima_beasiswa', $data);
    }

    function detailPenerimaBeasiswa () {
        cek_session_akses('beasiswa',$this->session->id_session);

        $nim                    = $this->uri->segment(3);
        $id_identitas_mahasiswa = $this->uri->segment(4);
        $id_identitas_wali      = $this->uri->segment(5);
        $id_dokumen             = $this->uri->segment(6);

        $data['status_beasiswa']        = $this->db->select('*')
                                            ->from('penerima_beasiswa')
                                            ->join('beasiswa', 'penerima_beasiswa.id_beasiswa = beasiswa.id', 'inner')
                                            ->where('nim', $nim)
                                            ->get()
                                            ->row();
        $data['identitas_mahasiswa']    = $this->db->get_where('identitas_mahasiswa', "id = $id_identitas_mahasiswa", 1, 0)->row();
        $data['identitas_wali']         = $this->db->get_where('identitas_wali', "id = $id_identitas_wali", 1, 0)->row();
        $data['dokumen_beasiswa']       = $this->db->get_where('dokumen_beasiswa', "id = $id_dokumen", 1, 0)->row();

        $this->template->load('administrator/template','administrator/mod_beasiswa/detail_penerima_beasiswa', $data);

    }

    function unduhDokumenBeasiswa () {
        cek_session_akses('beasiswa',$this->session->id_session);
        ...
        // download file
        force_download("asset/beasiswa/". md5($input->id) ."_". str_replace(' ', '_', $input->nama) ."/". $input->nim. "-". str_replace(' ', '_', $input->nama_mahasiswa) ."/$input->dokumen", null);
    }

    function ubahStatusPenerimaBeasiswa () {
        cek_session_akses('beasiswa',$this->session->id_session);

        $input = (object) $this->input->post(null, true);

        $this->db->where('nim', $input->nim);
        $this->db->update('penerima_beasiswa', $input);

        redirect($this->uri->segment(1).'/penerimabeasiswa');
    }
    ...
    function tambahBeasiswa () {
        ...
        if (!$_POST) {
            $this->template->load('administrator/template','administrator/mod_beasiswa/tambah_beasiswa', $data);
            return;
        }

        $input = (object) $this->input->post(null, true);

        $this->db->insert('beasiswa', $input);
        
        redirect($this->uri->segment(1).'/beasiswa');
    }
    ...
    function editBeasiswa () {
        ...
        $data['input'] = $this->db->get_where('beasiswa', ['id' => $id], 1, 0)->row();

        if (!$_POST) {
            $this->template->load('administrator/template','administrator/mod_beasiswa/edit_beasiswa', $data);
            return;
        }

        $input = (object) $this->input->post(null, true);

        $this->db->update('beasiswa', $input, "id = $id");

        if (is_dir('./asset/beasiswa/'.md5($id).'_'.str_replace(' ', '_', $data['input']->nama))) {
            rename("./asset/beasiswa/".md5($id)."_".str_replace(' ', '_', $data['input']->nama)."", "./asset/beasiswa/".md5($id)."_".str_replace(' ', '_', $input->nama)."");
        }
    ...
    function hapusBeasiswa () {
		...
		$id = array('id' => $this->uri->segment(3));
        $this->model_app->delete('beasiswa', $id);
        
		redirect($this->uri->segment(1).'/beasiswa');
    }

    function hapusPenerimaBeasiswa () {
        cek_session_akses('sekolah',$this->session->id_session);

		$nim = array('nim' => $this->uri->segment(3));
        $this->model_app->delete('penerima_beasiswa', $nim);

        $idMahasiswa = array('id' => $this->uri->segment(4));
        $this->model_app->delete('identitas_mahasiswa', $idMahasiswa);

        $idWali = array('id' => $this->uri->segment(5));
        $this->model_app->delete('identitas_wali', $idWali);

        $idDokumen = array('id' => $this->uri->segment(6));
        $this->model_app->delete('dokumen_beasiswa', $idDokumen);
        
		redirect($this->uri->segment(1).'/penerimabeasiswa');
    }
    
    // Controller Modul Sekolah
    function sekolah () {
        cek_session_akses('sekolah',$this->session->id_session);
        $data['record'] = $this->db->get('sekolah')->result();
    ...
    function tambahSekolah () {
        ...
        if (!$_POST) {
            $this->template->load('administrator/template','administrator/mod_sekolah/tambah_sekolah');
        } else {
            $filename = $this->input->post('tingkat').date('YmdHis');
            // upload file
            $upload = $this->model_app->uploadExcel('data_sekolah', $filename);
            ...
            if ($upload['result'] === 'failed') {
                ...
                $loadexcel = $excelreader->load('asset/files/data_sekolah/'.$upload.''); // Load file yang telah diupload ke folder excel
                ...
                $insert = $this->model_app->insertBatch('sekolah', $data);
                redirect($this->uri->segment(1).'/sekolah');
            }
        }
    }

    function unggahFotoSekolah () {
        cek_session_akses('sekolah', $this->session->id_session);
        ...
        $filename   = url_title('sekolah'.$npsn).date('YmdHis').'';

        $this->load->library('upload'); // Load librari upload
    
        $config['upload_path']      = "./asset/foto_sekolah";
        $config['allowed_types']    = 'jpg|JPEG|jpeg|png|PNG|svg';
        ...
        if(!$this->upload->do_upload('file')){ // Lakukan upload dan Cek jika proses upload berhasil
            echo '<script>alert(`Gagal unggah foto`)';
            redirect($this->uri->segment(1).'/sekolah');
        }
    ...
    function editSekolah () {
        ...
        $data['input'] = $this->db->get_where('sekolah', ['npsn' => $this->uri->segment(3)], 1, 0)->row();

        if (!$_POST) {
            $this->template->load('administrator/template','administrator/mod_sekolah/edit_sekolah', $data);
        } else {
            $input = (object) $this->input->post(null, true);
            ...
            if (!empty($_FILES) && $_FILES['foto']['name'] !== '') {
                ...
                if ($this->upload->do_upload('foto')) {
                    if ($data['input']->foto !== '') {
                        $this->deleteImage($data['input']->foto);
                    }
                    $input->foto   = $this->upload->data('file_name');
                } else {
                    echo '<script>alert(`Gagal upload foto sekolah`)</script>';
                    redirect(base_url('administrator/editsekolah'));
                }
            } else {
                $input->foto = $data['input']->foto;
            }

            if ($this->db->where('npsn', $data['input']->npsn)->update('sekolah', $input)) {
                redirect($this->uri->segment(1).'/sekolah');
            } else {
                echo '<script>alert(`Gagal input sekolah`)</script>';
                redirect(base_url('administrator/editsekolah'));
            }
        }
    }
    ...
    {
        if (file_exists("./asset/foto_sekolah/$fileName")) {
            unlink("./asset/foto_sekolah/$fileName");
        }
    }
    ...
    function hapusSekolah () {
		...
		$npsn = array('npsn' => $this->uri->segment(3));
        $this->model_app->delete('sekolah',$npsn);

		redirect($this->uri->segment(1).'/sekolah');
    }

    function hapusSemuaSekolah () {
        cek_session_akses('sekolah',$this->session->id_session);

        if ($_POST) {
            $this->db->empty_table('sekolah');
        }
    ...
    function guru () {
        cek_session_akses('sekolah',$this->session->id_session);
        $data['record'] = $this->db->query("SELECT guru.nip,
                                    guru.nuptk,
                                    guru.nama,
                                    guru.jenis_ptk,
                                    guru.agama,
                                    ...
                                    FROM guru
                                    INNER JOIN sekolah
                                    ON guru.npsn = sekolah.npsn")
                            ->result();
		$this->template->load('administrator/template','administrator/mod_sekolah/guru', $data);
    }
    ...
    function tambahGuru () {
        ...
        if (!$_FILES) {
            $this->template->load('administrator/template','administrator/mod_sekolah/tambah_guru');
        } else {
            //$filename = 'guru'.date('YmdHis').'';
            // upload file
            $upload = $this->model_app->uploadExcel('data_guru', $filename);
            ...
            if ($upload['result'] === 'failed') {
                ...
                $excelreader = new PHPExcel_Reader_Excel2007();
                $loadexcel = $excelreader->load('asset/files/data_guru/'.$upload.''); // Load file yang telah diupload ke folder excel
                $sheet = $loadexcel->getActiveSheet()->toArray(null, true, true ,true);
                ...
                //    $date = DateTime::createFromFormat('d/m/Y', $value['tanggal_lahir'] );
                //    $data[$key]['tanggal_lahir'] = $date->format('Y-m-d');
                //}
                
                // Panggil fungsi insert_multiple yg telah kita buat sebelumnya di model
                $insert = $this->model_app->insertBatch('guru', $data);
    ...
    function hapusGuru () {
		...
		$nip = array('nip' => $this->uri->segment(3));
        $this->model_app->delete('guru', $nip);
    ...
    function siswa () {
        cek_session_akses('sekolah',$this->session->id_session);
        $data['record'] = $this->db->query("SELECT siswa.nis,
                                    siswa.nama,
                                    siswa.jenis_kelamin,
                                    siswa.tanggal_lahir,
                                    siswa.agama,
                                    ...
                                    FROM siswa
                                    INNER JOIN sekolah
                                    ON siswa.npsn = sekolah.npsn")
                                ->result();
        $this->template->load('administrator/template','administrator/mod_sekolah/siswa', $data);
    }
    ...
    function tambahSiswa () {
        ...
        if (!$_FILES) {
            $this->template->load('administrator/template','administrator/mod_sekolah/tambah_siswa');
        } else {
            $filename = 'guru'.date('YmdHis').'';
            // upload file
            $upload = $this->model_app->uploadExcel('data_siswa', $filename);
            ...
            if ($upload['result'] === 'failed') {
                ...
                $excelreader = new PHPExcel_Reader_Excel2007();
                $loadexcel = $excelreader->load('asset/files/data_siswa/'.$upload.''); // Load file yang telah diupload ke folder excel
                $sheet = $loadexcel->getActiveSheet()->toArray(null, true, true ,true);
                ...
                // Panggil fungsi insert_multiple yg telah kita buat sebelumnya di model
                $insert = $this->model_app->insertBatch('siswa', $data);
    ...
    function editsiswa(){
        ...
        if (isset($_POST['submit'])){
            $data = array('nama'=>$this->input->post('a'),
                        'kelas'=>$this->input->post('b'),
                        'jenis_kelamin'=>$this->input->post('c'),
                        'tempat_lahir'=>$this->input->post('d'),
                        'tanggal_lahir'=>$this->input->post('e'),
                        'agama'=>$this->input->post('f'),
                        'alamat'=>$this->input->post('g'));
            $where = array('nis' => $this->input->post('id'));
            $this->model_app->update('siswa', $data, $where);
            redirect($this->uri->segment(1).'/siswa');
        }else{
            $proses = $this->model_app->edit('siswa', array('nis' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template','administrator/mod_sekolah/view_siswa_edit',$data);
        }
    }
    ...
    function hapusSiswa () {
		...
		$nis = array('nis' => $this->uri->segment(3));
        $this->model_app->delete('siswa', $nis);
