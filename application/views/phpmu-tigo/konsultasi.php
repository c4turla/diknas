<?php if ($this->uri->segment(3) == ''){ $stat = 'Aduan'; $id = '0'; }else{ $stat = 'Jawaban'; $id = $this->uri->segment(3); } ?>
						...
						<form action="<?php echo base_url(); ?>konsultasi/reply" method="POST" onSubmit="return validasi(this)" id="form_komentar">
							...
							<?php 
								...
								if ($this->uri->segment(3) != ''){  
									echo "<p><label for='c_email'><b>Aduan</b><span class='required'></span></label>
							...
							<?php if ($this->uri->segment(3) == ''){ ?>
								<p><input type="submit" name="submit" class="styled-button" value="Kirimkan Pertanyaan" onclick="return confirm('Yakin ingin mengirimkan Aduan ini ?')"/></p>
							<?php }else{ ?>
